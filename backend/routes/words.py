...
@router.get("/export")
def export_words(db: Session = Depends(get_db), user=Depends(current_user)):
    """So'zlarni eksport qilish (text format)."""
    from models import Word
    words = db.query(Word).filter(Word.user_id == user.id).order_by(Word.created_at.desc()).all()
    lines = [f"{w.word} - {w.translation}" for w in words]
    return {
        "ok": True,
        "total": len(words),
        "content": "\n".join(lines),
        "filename": f"brainbridge_words_{len(words)}.txt"
    }


@router.delete("/clear-all")
def clear_all_words(db: Session = Depends(get_db), user=Depends(current_user)):
    """Barcha so'zlarni o'chirish."""
    from models import Word
    count = db.query(Word).filter(Word.user_id == user.id).delete()
    db.commit()
...
def review(word_id: int, body: ReviewIn, db: Session = Depends(get_db), user=Depends(current_user)):
    try:
        ...
        # Note: XP is already awarded inside advance(), no double-counting
        return result
    except ValueError as e:
