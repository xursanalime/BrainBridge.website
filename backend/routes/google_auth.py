...
async def google_callback(code: str | None = None,
    ...
    is_admin_str = "1" if getattr(user, 'is_admin', False) else "0"
    response = RedirectResponse(f"/?google_token={access_token}&user_id={user.id}&email={email}&is_admin={is_admin_str}")
    response.set_cookie(
        key="refresh_token",
        value=refresh_token,
        httponly=True,
